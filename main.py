...
def sequence(*functions):
    for function in functions:
        function()
def update(y,m,tx,curdate):
    calstr=calendar.month(y,m)
    tx.configure(state=Tkinter.NORMAL)
    tx.delete('0.0',Tkinter.END)
    tx.insert(Tkinter.INSERT,calstr)
    for i in range(2,9):
        tx.tag_add("others",'{}.0'.format(i),'{}.end'.format(i))
...
ctx=Tkinter.Text(top,padx=10,pady=10,bg="#f3e9ae",relief=Tkinter.FLAT,height=9,width=20)
def nextb():
    localv=globals()
    localv['monthInt']+=1
...
def prevb():
    ...
    update(localv['yearInt'],localv['monthInt'],localv['ctx'],localv['curtime'])
def okcall():
    if (year.get().isdigit()and month.get().isdigit())and((int(year.get())<10000 and int(year.get())>0) and (int(month.get())<13 and int(month.get()))):
        localv=globals()
